...
// 创建员工表，如果表不存在则创建，存在则不做任何操作，默认员工表有id, name, gender, birthday, address字段, 其中id为主键
int create_employee_table(sqlite3* db);
// 增删改查
int add_employee(sqlite3* db, Employee* employee);
int delete_employee(sqlite3* db, int id);
int update_employee(sqlite3* db, Employee* employee);
Employee* employee_find_by_id(sqlite3* db, int id);
Employee* employee_find_by_name(sqlite3* db, char* name);
